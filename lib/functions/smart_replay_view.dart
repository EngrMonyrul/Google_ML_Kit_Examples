...
class _SmartReplyViewState extends State<SmartReplyView> {
            ...
            child: ListView(
              children: [
                ...
                Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
                  ...
                  ElevatedButton(onPressed: _suggestReplies, child: Text('Get Suggest Replies')),
                ]),
                SizedBox(height: 30),
                if (_suggestions != null) Text('Status: ${_suggestions!.status.name}'),
                if (_suggestions != null && _suggestions!.suggestions.isNotEmpty)
                  for (final suggestion in _suggestions!.suggestions) Text('\t $suggestion'),
              ],
            ),
          ),
        ),
  ...
  void _addMessage(TextEditingController controller, bool localUser) {
    ...
    if (controller.text.isNotEmpty) {
      if (localUser) {
        _smartReply.addMessageToConversationFromLocalUser(controller.text, DateTime.now().millisecondsSinceEpoch);
      } else {
        _smartReply.addMessageToConversationFromRemoteUser(
            controller.text, DateTime.now().millisecondsSinceEpoch, 'userZ');
      }
      controller.text = '';
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Message added to the conversation')));
    } else {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Message can\'t be empty')));
    }
  }
