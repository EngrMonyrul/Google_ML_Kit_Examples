...
class _EntityExtractionViewState extends State<EntityExtractionView> {
            ...
            child: Column(
              children: [
                ...
                Row(
                    ...
                    children: [ElevatedButton(onPressed: _extractEntities, child: Text('Extract Entities'))]),
                const SizedBox(height: 20),
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceAround,
                  children: [
                    ElevatedButton(onPressed: _downloadModel, child: Text('Download Model')),
                    ElevatedButton(onPressed: _deleteModel, child: Text('Delete Model')),
                  ],
                ),
                SizedBox(height: 20),
                Row(
                    mainAxisAlignment: MainAxisAlignment.spaceAround,
                    children: [ElevatedButton(onPressed: _isModelDownloaded, child: Text('Check download'))]),
                const SizedBox(
                  height: 30,
                ),
                Container(
                ...
                ListView.builder(
                  ...
                  itemBuilder: (context, index) => ExpansionTile(
                      ...
                      children: _entities[index].entities.map((e) => Text(e.toString())).toList()),
                ),
              ],
            ),
          ),
  ...
  Future<void> _downloadModel() async {
    Toast().show('Downloading model...',
        _modelManager.downloadModel(_language.name).then((value) => value ? 'success' : 'failed'), context, this);
  }

  Future<void> _deleteModel() async {
    Toast().show('Deleting model...',
        _modelManager.deleteModel(_language.name).then((value) => value ? 'success' : 'failed'), context, this);
  }

  Future<void> _isModelDownloaded() async {
    Toast().show(
        'Checking if model is downloaded...',
        _modelManager.isModelDownloaded(_language.name).then((value) => value ? 'downloaded' : 'not downloaded'),
        context,
        this);
  }
